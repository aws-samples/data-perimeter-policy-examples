...
2. Replace the placeholder values in the example policies based on your definition of trusted identities, trusted resources, and expected networks:
    ...
    * Replace `<ecr-account-id>` with the ID of the AWS account that owns [Amazon Elastic Container Registry (Amazon ECR)](https://aws.amazon.com/ecr/) repositories that you require to be used in your environment.
    * Replace `<service-account-id>` with the ID of the AWS account that owns the particular resource that you require to be used within your environment.
    ...
    * Replace `<excluded-principal-arn-a>` and `<excluded-principal-arn-b>` with the ARNs of IAM principals that should be exempted from network restrictions for break-glass console access to AWS.     
